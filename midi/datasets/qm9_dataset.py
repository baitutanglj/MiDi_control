...
from midi.datasets.dataset_utils import mol_to_torch_geometric, remove_hydrogens, Statistics, mol_to_control_data
...
class QM9Dataset(InMemoryDataset):
    ...
    def download(self):
        ...
        try:
            ...
            file_path = download_url(self.raw_url2, self.raw_dir, filename='uncharacterized.txt')
        ...
        excluded_strings = []
        with open(os.path.join(self.raw_dir, 'uncharacterized.txt')) as f:
            lines = f.readlines()
            excluded_strings = [line.split()[0]
                                for line in lines if len(line.split()) > 0]

        excluded_idxs = [int(idx) - 1 for idx in excluded_strings if idx.isdigit()]
