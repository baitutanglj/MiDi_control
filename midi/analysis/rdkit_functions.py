...
class Molecule:
    def __init__(self, atom_types, bond_types, positions, charges, atom_decoder, template_idx=None):
    ...
    def build_molecule(self, atom_decoder, verbose=False):
        ...
        if self.template_idx is not None:
            idx = str(self.template_idx.cpu().item()) if type(self.template_idx)==torch.Tensor else str(self.template_idx)
            mol.SetProp('template_idx', idx)
        return mol
