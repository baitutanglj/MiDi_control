...
class XEyTransformerLayer(nn.Module):
    ...
    def __init__(self, dx: int, de: int, dy: int, n_head: int, dim_ffX: int = 2048,
        ...
        self.norm_pos1 = SE3Norm(eps=layer_norm_eps, **kw) #E(3) Norm
    ...
    def forward(self, features: utils.PlaceHolder):
        ...
        """
        X = features.X#torch.Size([64, 17, 256])
        E = features.E#torch.Size([64, 17, 17, 64])
        y = features.y#torch.Size([64, 128])
        pos = features.pos#torch.Size([64, 17, 3])
        node_mask = features.node_mask#torch.Size([64, 17])
        ...
        # new_pos = pos + vel
        new_pos = self.norm_pos1(vel, x_mask) + pos#torch.Size([64, 17, 3])
...
class NodeEdgeBlock(nn.Module):
    ...
    def forward(self, X, E, y, pos, node_mask):
        ...
        pos = pos * x_mask
        norm_pos = torch.norm(pos, dim=-1, keepdim=True)         # bs, n, 1  #torch.Size([64, 17, 1])
        normalized_pos = pos / (norm_pos + 1e-7)                 # bs, n, 3

        pairwise_dist = torch.cdist(pos, pos).unsqueeze(-1).float() #||pi-pj||   #torch.Size([64, 17, 17, 1])
        cosines = torch.sum(normalized_pos.unsqueeze(1) * normalized_pos.unsqueeze(2), dim=-1, keepdim=True) #cos(pi, pj)  #torch.Size([64, 17, 17, 1])
        pos_info = torch.cat((pairwise_dist, cosines), dim=-1)#torch.Size([64, 17, 17, 2])

        norm1 = self.lin_norm_pos1(norm_pos)             # bs, n, de #torch.Size([64, 17, 64])
        norm2 = self.lin_norm_pos2(norm_pos)             # bs, n, de
        dist1 = F.relu(self.lin_dist1(pos_info) + norm1.unsqueeze(2) + norm2.unsqueeze(1)) * e_mask1 * e_mask2  #||P||
        ...
        vel = (messages * delta_pos).sum(dim=2) * x_mask #torch.Size([64, 17, 3])
        vel = utils.remove_mean_with_mask(vel, node_mask) #torch.Size([64, 17, 3])
