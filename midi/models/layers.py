...
class PositionsMLP(nn.Module):
    ...
    def forward(self, pos, node_mask):
        ...
        new_pos = pos * new_norm / (norm + self.eps)#torch.Size([64, 17, 3])
        ...
        return new_pos#torch.Size([64, 17, 3])


class PositionsMLP_Control(nn.Module):
    def __init__(self, hidden_dim, eps=1e-5):
        super().__init__()
        self.eps = eps
        self.mlp = nn.Sequential(nn.Linear(1, hidden_dim), nn.ReLU(),
                                 zero_module(nn.Linear(hidden_dim, 1)))
