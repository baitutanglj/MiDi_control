...
def cosine_beta_schedule_discrete(timesteps, nu_arr, s=0.008):
    ...
    nu_arr = np.array(nu_arr)  # (components, )  # X, charges, E, y, pos #array([2.5, 1. , 1. , 1.5, 1. ])
    nu_arr = np.expand_dims(nu_arr, 1)  # ((components, 1))

    alphas_cumprod = np.cos(0.5 * np.pi * (((x / steps) ** nu_arr) + s) / (1 + s)) ** 2  # ((components, steps)) #shape:(5, 502)
...
def sample_discrete_features(probX, probE, prob_charges, node_mask):
    ...
    probX = probX.reshape(bs * n, -1)       # (bs * n, dx_out) #torch.Size([1088, 5])
    prob_charges = prob_charges.reshape(bs * n, -1)  # (bs * n, dx_out) #torch.Size([1088, 3])

    # Sample X
    X_t = probX.multinomial(1)                                  # (bs * n, 1) #torch.Size([1088, 1])
    X_t = X_t.reshape(bs, n)     # (bs, n) #torch.Size([64, 17])

    charges_t = prob_charges.multinomial(1)
    charges_t = charges_t.reshape(bs, n) #torch.Size([64, 17])
    ...
    probE = probE.reshape(bs * n * n, -1) #torch.Size([18496, 5])

    # Sample E
    E_t = probE.multinomial(1).reshape(bs, n, n)   # (bs, n, n)
    E_t = torch.triu(E_t, diagonal=1)
    E_t = (E_t + torch.transpose(E_t, 1, 2)) #torch.Size([64, 17, 17])
