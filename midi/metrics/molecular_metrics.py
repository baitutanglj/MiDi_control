...
class SamplingMetrics(nn.Module):
    def __init__(self, train_smiles, dataset_infos, test, template=None):
        super().__init__()
    ...
    def evaluate(self, generated, local_rank):
        ...
        return all_smiles, all_valid_mols
    ...
    def __call__(self, molecules: list, name, current_epoch, local_rank):
        ...
        all_generated_smiles, all_valid_mols = self.evaluate(molecules, local_rank=local_rank)
        ...
        # textfile.write('\n'.join(all_generated_smiles))
        # textfile.close()
        # Save in the root folder if test_model
        current_path = os.getcwd()
        result_dir = 'test' if self.test else 'val'
        result_path = os.path.join(current_path, f"{result_dir}/epoch{current_epoch}/")
        os.makedirs(result_path, exist_ok=True)
        filename = f"{result_path}/final_smiles_GR{local_rank}_{0}.txt"
        sdf_filename = f"{result_path}/final_mol_GR{local_rank}_{0}"
        for i in range(2, 10):
            if os.path.exists(filename):
                filename = f"{result_path}/final_smiles_GR{local_rank}_{i}.txt"
                sdf_filename = f"{result_path}/final_mol_GR{local_rank}_{i}"
            else:
                break
        ...
        for i in range(len(self.template)):
            with Chem.SDWriter(f"{sdf_filename}_template{i}.sdf")as f:
                f.write(self.template[i])
                for mol in all_valid_mols[str(i)]:
                    f.write(mol)
        print(f'All mol saved on rank {local_rank}')
        # if self.test:
        #     filename = f'final_smiles_GR{local_rank}_{0}.txt'
        #     sdf_filename = f'final_smiles_GR{local_rank}_{0}.sdf'
        #     for i in range(2, 10):
        #         if os.path.exists(filename):
        #             filename = f'final_smiles_GR{local_rank}_{i}.txt'
        #             sdf_filename = f'final_smiles_GR{local_rank}_{i}.sdf'
        #         else:
        #             break
        #     with open(filename, 'w') as fp:
        #         fp.write('\n'.join(all_generated_smiles))
...
if __name__ == '__main__':
    ...
    def main(cfg: omegaconf.DictConfig):
        ...
        sampling_metrics = SamplingMetrics(train_smiles=train_smiles, dataset_infos=dataset_infos, test=True,
                                           template=datamodule.test_dataset.template_data)
